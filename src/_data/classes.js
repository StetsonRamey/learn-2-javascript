module.exports = {
  // Classes that will be added to different elements
  html: 'font-normal font-sans leading-relaxed antialiased',
  body:
    'bg-gray-100 text-bodytext text-lg md:text-xl overflow-x-hidden flex flex-col min-h-screen',
  bodyGuide: 'px-8 lg:grid lg:grid-cols-2 lg:gap-4',
  navbar:
    'w-full px-8 fixed top-0 left-0 shadow-nav z-10 bg-gray-100 text-bodytext md:shadow-none md:static md:mt-8 md:mb-20 md:px-0',
  main: 'flex flex-col flex-grow',
  '2Col': 'grid-cols-2 col-gap-4',
  contact_button: 'btn btn-primary text-white dark:text-cool-gray-900',
  footer: 'my-20 mx-auto max-w-screen-xl',
  tag: 'text-sm py-1 px-2 mr-1 rounded-sm',
  jsTag: 'text-gray-900 bg-yellow-400 hover:text-gray-800 hover:bg-yellow-600',
  tailwindTag:
    'text-gray-900 bg-teal-600 hover:text-gray-800 hover:bg-teal-300',
  '11tyTag': 'text-gray-100 bg-gray-800 hover:text-gray-800 hover:bg-gray-500',
};
